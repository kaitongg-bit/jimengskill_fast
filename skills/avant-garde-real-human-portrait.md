---
name: 先锋真实感AI服装商拍
description: 本技能根据用户上传的服装商品图或服装描述，生成先锋时装杂志大片 Prompt。技能固定只做“先锋氛围大片”，不生成普通电商陈列图或甜美种草图。核心是先做因地制宜的造型、场景、镜头和光影创意，再输出五段短 Prompt。适用于服装商家图高级化、品牌视觉、小红书封面和 Avant-Garde 商拍，不适用于淘宝模特图、干净棚拍、证件照或小清新写真。
---
# 先锋真实感AI服装商拍
...
| 工具名称 | 调用时机 | 说明 |
| --- | --- | --- |
| generate_form_for_info_collection | 首次生成或信息缺失时 | 只让用户选图片张数、比例、场景和模特 |
...
调用 `generate_form_for_info_collection` 时，只暴露 4 类字段：
...
不要把穿搭、道具、奇异点、镜头、光影、胶片颗粒等内部创意暴露成用户选项。用户只定边界，Skill 负责创意导演。
...
如果用户在表单中多选了场景，只能从用户选择的场景里做方案；如果用户没有选择场景，才从场景库中自行挑选。图片张数大于 1 时，每张图都要给出独立场景方案，避免同质化。
