---
name: 人像捏脸提示词生成
description: 根据用户选择或自定义的脸谱清单、气质、年龄段、性别表达和参考图，生成可用于即梦 text2image/image2image 的人像捏脸 Prompt。技能把猫系脸、狐系脸、反派脸、清冷脸、幼态脸等抽象审美词拆成脸型、眼型、鼻子、嘴唇、面部折叠度、三庭五眼、气质和妆容，并允许用户用自然语言微调五官，不处理明星仿脸、未授权真人复刻、医学诊断或面相判断。
---
# 人像捏脸提示词生成
...
| 工具名称 | 调用时机 | 说明 |
| --- | --- | --- |
| generate_form_for_info_collection | 缺少目标脸谱、气质或出图设置时 | 用脸谱清单收集预设选择，并允许用户自定义微调 |
...
如果用户没有明确目标，调用 `generate_form_for_info_collection`。脸谱必须做成清单，给默认选项，但允许用户自定义输入调整：
...
2. 脸谱自定义微调：可填写“眼型再拉长一点、红唇加重一点、中庭不要太长、脸不要太尖、少一点攻击性”等
3. 气质方向：冷感 / 甜酷 / 慵懒 / 智性 / 厌世 / 纯净 / 攻击性 / 疏离 / 自定义
4. 年龄段：18-22成年 / 23-28 / 29-35 / 35+
5. 性别表达：女性化 / 中性 / 男性化 / 自定义
6. 出图类型：正面头像 / 半身肖像 / 时装大片 / 角色设定照
7. 画面比例：1:1 / 3:4 / 4:5 / 9:16
8. 是否有参考图：无 / 有脸部参考 / 有妆容参考 / 有发型参考
```

不要把三庭五眼、鼻翼宽度、眼裂长度等拆成大量专业选项让用户填。用户只需要选脸谱清单和写自然语言微调，Skill 再把它翻译成专业五官参数。

自定义微调优先级最高：用户写的“不要太媚”“脸不要太尖”“眼型再拉长”“红唇加重”必须覆盖或修正脸谱预设。

### Phase 2: 脸谱清单解码

先输出一个短表，说明本次清单选择和自定义微调如何落到五官：
